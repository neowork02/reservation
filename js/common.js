...
$(function(){
	...
	$.fn.tabAnchor = function(){
		$.each(this, function(i,v){
			$(v).closest('.tab_anchor').find('li').removeClass('on');
	...
	/* Step_list */
	$('.step_list .list .img').imgLiquid({fill:true, horizontalAlign:"center", verticalAlign:"top"});
	$('.step_list a').bind('mouseover',function(){
		$('.step_list').removeClass('idx');
		$('.step_list .list').removeClass('on');
		$(this).closest('.list').addClass('on');
	});
	$('.step_list a').bind('mouseleave',function(){
		$('.step_list').addClass('idx');
		$('.step_list .list').removeClass('on');

	});
	$('.step_list a').click(function(){
		return false;
	});
});
...
function accordion(me) {
    ...
    var item = $(me).closest('.acc_item');
    var cont = item.find('.acc_cont');
    if(!item.hasClass('active')){
        TweenMax.to(item.siblings('.active').find('.acc_cont'), 0.3, {height:0, onComplete:function() {
            item.siblings('.active').removeClass('active');
            //console.log($(me).position().top);
            $(window).animate({
                scrollTop: $(me).position().top - 100
            }, 300);
        }});
        item.addClass('active');
        TweenMax.fromTo(cont, 0.3, {height:0}, {height:cont.children().outerHeight()});
    }else{
        TweenMax.to(cont, 0.3, {height:0, onComplete:function() {
